...
ADIOSSave_Reduce::ADIOSSave_Reduce(MPI_Comm const * _parent_comm, int const _gid,
		...
		ADIOSManager *_iomanager, cci::common::LogSession *_logsession) :
...
//	if (status == Communicator_I::DONE) cci::common::Debug::print("%s call_count = %ld, status = %d, max_iter = %d, local_iter = %d, buffer size = %ld\n",
...
//	cci::common::Debug::print("%s calling ADIOS to persist the files\n", getClassName());
	writer->persist(local_iter);
	//MPI_Barrier(comm);
	writer->clearBuffer();
	++local_iter;

//	cci::common::Debug::print("%s persist completes at rank %d\n", getClassName(), rank);
