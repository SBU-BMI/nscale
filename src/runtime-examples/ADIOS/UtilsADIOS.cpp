...
ADIOSManager::ADIOSManager(const char* configfilename, boost::program_options::variables_map &_vm,
		int _rank, MPI_Comm &_comm, cci::common::LogSession *session) :
...
ADIOSManager::ADIOSManager(const char* configfilename, std::string _transport,
		int _rank, MPI_Comm &_comm, cci::common::LogSession *session, bool _grouped, bool _gapped) :
...
ADIOSWriter* ADIOSManager::allocateWriter(
	...
	long long t2 = ::cci::common::event::timestampInUS();
	if (this->logsession != NULL) this->logsession->log(cci::common::event(0, std::string("ADIOS Writer alloc"), t1, t2, std::string(), ::cci::common::event::MEM_IO));
...
//	printf("cleaned up writer %d \n", w->local_rank);
	long long t2 = ::cci::common::event::timestampInUS();
	if (this->logsession != NULL) this->logsession->log(cci::common::event(0, std::string("ADIOS Writer free"), t1, t2, std::string(), ::cci::common::event::MEM_IO));
...
//		pg_tileInfo_count(0), pg_imageName_bytes(0), pg_filename_bytes(0), pg_image_bytes(0),
	...
	//cci::common::Debug::print("Local_tile_buffer at %p with capacity %d\n", tile, local_tile_capacity);
...
//				tile_offset[i], imageName_size[i], imageName_offset[i], sourceTileFile_size[i], sourceTileFile_offset[i]);
	...
	if (this->logsession != NULL) this->logsession->log(cci::common::event(0, ss.str(), t1, t2, std::string("1"), ::cci::common::event::NETWORK_IO));
	ss.str(std::string());
	t1 = ::cci::common::event::timestampInUS();
...
#include "gwrite_tileInfo.ch"
	...
	if (this->logsession != NULL) this->logsession->log(cci::common::event(0, ss.str(), t1, t2, std::string(len), ::cci::common::event::ADIOS_CLOSE));
	ss.str(std::string());
