...
POSIXRawSave::~POSIXRawSave() {
	cci::common::Debug::print("%s destructor:  wrote out %d over %d iters\n", getClassName(), local_total, local_iter);
...
//		cci::common::Debug::print("TEST start input status = %d\n", input_status);
...
//	if (test_input_status == DONE)
//		cci::common::Debug::print("TEST 1 input status = %d\n", input_status);
...
//		cci::common::Debug::print("%s call_count = %ld, status = %d, max_iter = %d, local_iter = %d, buffer size = %d\n", getClassName(), c, status, max_iter, local_iter, inputBuf->debugBufferSize());
//	}



//	if (test_input_status == DONE)
//		cci::common::Debug::print("TEST 2 input status = %d\n", input_status);

	t2 = ::cci::common::event::timestampInUS();
	//if (this->logsession != NULL) this->logsession->log(cci::common::event(0, std::string("IO MPI update iter"), t1, t2, std::string(), ::cci::common::event::NETWORK_IO));
	...
	//	cci::common::Debug::print("%s rank %d max iter = %d, local_iter = %d\n", getClassName(), rank, max_iter, local_iter);
	...
	while (max_iter > local_iter) {
		//cci::common::Debug::print("%s write out: IO group %d rank %d, write iter %d, max_iter = %d, tile count %d\n", getClassName(), groupid, rank, local_iter, max_iter, inputBuf->debugBufferSize());
		process();
	}

//	if (test_input_status == DONE)
//		cci::common::Debug::print("TEST end input status = %d\n", input_status);

	//cci::common::Debug::print("%s rank %d returning input status %d\n", getClassName(), rank, input_status);
	return status;
}
...
int POSIXRawSave::process() {
	...
	while (this->inputBuf->canPop()) {
		...
		if (input != NULL) {
			...
			std::string tmpfn = cci::common::FileUtils::replaceDir(sourcefn, cci::common::FileUtils::getDir(sourcefn), outdir);
			std::string outfn = cci::common::FileUtils::replaceExt(tmpfn, cci::common::FileUtils::getExt(tmpfn), "out.raw");
...
//			printf("FILESNAMES: source %s, temp %s, out %s\n", sourcefn.c_str(), tmpfn.c_str(), outfn.c_str());
	...
	if (this->logsession != NULL) this->logsession->log(cci::common::event(0, std::string("IO POSIX Write"), t1, t2, std::string(len), ::cci::common::event::FILE_O));
