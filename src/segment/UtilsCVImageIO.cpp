...
namespace cv {
	...
	IntermediateResultWriter::IntermediateResultWriter(const std::string &pref, const std::string &suf, std::vector<int> &selStages) {
		prefix.assign(pref);
		suffix.assign(suf);
...
#else
	void IntermediateResultWriter::saveIntermediate(const ::cv::gpu::GpuMat& intermediate, const int stage) { throw_nogpu(); }
#endif
