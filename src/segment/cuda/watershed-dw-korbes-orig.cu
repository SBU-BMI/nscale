...
//------------------------------------------------------------------------------
texture<unsigned char, 2, cudaReadModeElementType> timg;
                                      ...
                                      int conn )
{
    ...
    cudaMallocArray(&cuarrayImg, &desc8, w, h);
    ...
    cudaMemcpyToArray(cuarrayImg, 0, 0, hdataIn, w * h * sizeof(unsigned char), cudaMemcpyDeviceToDevice);
    cudaBindTextureToArray(timg,cuarrayImg);
    cuIndexDescida<<<dimGrid, dimBlock>>>(gArrow, gDist, w, h,conn);
    ...
    do{
        ...
        cuPropagaDescida<<<dimGrid, dimBlock>>>(gArrow, gDist, w, h, conn, gCtrl);
    ...
    do{
        ...
        cuAgrupaPixel<<<dimGrid, dimBlock>>>(gDist, gCtrl,conn,w,h);
        cudaMemcpy(&hCtrl,gCtrl, sizeof(int),cudaMemcpyDeviceToHost);

        if(hCtrl)
        {
            cudaBindTexture(0,taux,gDist,sizei);
            cuPropagaIndex<<<dimGrid1, dimBlock1>>>(gArrow,gDist,w*h);
            cudaUnbindTexture(taux);

            cudaBindTexture(0,taux,gDist,sizei);
            cuLabel<<<dimGrid1, dimBlock1>>>(gArrow, w*h);
            cudaUnbindTexture(taux);
    ...
    cuAjustaVetor<<<dimGrid1, dimBlock1>>>(gArrow,gDist, w*h);
    cudaBindTexture(0,taux,gDist,sizei);
    cuPropagaIndex<<<dimGrid1, dimBlock1>>>(gArrow,gDist, w*h);
    cudaThreadSynchronize();
