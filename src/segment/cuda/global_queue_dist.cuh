...
int *distQueueBuildCaller(int rows, int cols, cv::gpu::PtrStep_<unsigned char> mask,cv::gpu::PtrStep_<int> nearestNeighbors, int &queue_size, cudaStream_t stream);
int distTransformPropagation( int *g_InputListPtr, int h_ListSize, cv::gpu::PtrStep_<unsigned char> mask , cv::gpu::PtrStep_<int> nearestNeighbors, int cols, int rows, int queue_increase_factor=2);
