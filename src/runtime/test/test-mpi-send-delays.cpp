...
// set up the manager/worker
	...
	int callCount = 0;
		...
		logsession->log(cci::common::event(0, std::string("setup"), t1, t2, std::string(), ::cci::common::event::NETWORK_WAIT));
...
//					printf("%d ended by %d \n", rank, node_id);
		...
		sprintf(notes, "%d", node_id);
					logsession->log(cci::common::event(0, std::string("worker end"), t1, t2, std::string(notes), ::cci::common::event::NETWORK_IO));
...
//					printf("%d receiving from %d\n", rank, node_id);
		...
		sprintf(notes, "%d", recv_size);
					...
					logsession->log(cci::common::event(0, std::string("recv"), t1, t2, std::string(notes), ::cci::common::event::NETWORK_IO));
				}
			...
			// call this with or without messages.  some nodes may not get a message and we want to make sure all processes call Allreduce the same number of times.
				...
				//	printf("%d allreduce\n", rank);
					...
					//logsession->log(cci::common::event(0, std::string("check to write"), t3, t4, std::string(), ::cci::common::event::NETWORK_WAIT));
		...
		sprintf(notes, "%d", data_size);
							...
							logsession->log(cci::common::event(0, std::string("write"), t3, t4, std::string(notes), ::cci::common::event::FILE_O));
		...
		for (int i = 0; i < output_num; ++i) {
			...
			sleep(1);
			t2 = ::cci::common::event::timestampInUS();
			logsession->log(cci::common::event(0, std::string("compute"), t1, t2, std::string(), ::cci::common::event::COMPUTE));
			...
			memset(notes, 0, 21);
		sprintf(notes, "%d", node_id);
			logsession->log(cci::common::event(0, std::string("send"), t1, t2, std::string(notes), ::cci::common::event::NETWORK_IO));
		...
		delete [] reqs;			
		t2 = ::cci::common::event::timestampInUS();
		logsession->log(cci::common::event(0, std::string("worker finish"), t1, t2, std::string(), ::cci::common::event::NETWORK_WAIT));
