...
do
	...
	mpirun -np $i --hostfile=${HOSTFILE} --bynode ${BIN_DIR}/nu-segment-scio.exe ${DATA_DIR}/ValidationSet ${DATA_DIR}/scio-val-mpi ${TOTAL_COUNT} 0-101 cpu 1 > ${OUT_DIR}/${OUT_PREFIX}.${i}.csv
done
