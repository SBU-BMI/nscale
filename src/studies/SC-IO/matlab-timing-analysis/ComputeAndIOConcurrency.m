function [ concurrencyAll loadAll nodes concurrencyPerNode loadPerNode procsPerNode] = ComputeAndIOConcurrency( header, type, startTimes, endTimes, location )
...
% per node
    ...
    loadPerNode = cell(length(nodes), 1);
    procsPerNode = zeros(length(nodes), 1, 'int32');
    for i = 1 : length(nodes)
       ...
       procsPerNode(i) = nodeProcs;
       [concurrencyPerNode{i, 1} loadPerNode{i, 1}]= computeConcurrency(header, type, startTimes(idx, :), endTimes(idx, :), nodeProcs);
    end

% all processes
    nodeProcs = length(unique(location{:, 1}));  % count the unique ranks.
    [concurrencyAll loadAll]  = computeConcurrency(header, type, startTimes, endTimes, nodeProcs);

end

function [ output load ] = computeConcurrency(header, type, stime, etime, cpuCount)
