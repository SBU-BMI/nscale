...
__device__ __inline__ void detectRuns512(ushort* pt, uchar* ct, uchar* ps, uchar* px, ushort* tbf, uint cp2)
{
	uint* ps4 = (uint*)ps;
	uint* pt2 = (uint*)pt;
...
__global__ void pass1_512(ushort* pt, uchar* ps, ushort* b,  ushort* b2, uint h, ushort* eb)
{
...
//		return;
	...
	px[cp>>1] = 0;
//	((ushort*)ct)[cp>>1] = 0;
	detectRuns512(lk0, ct, ps, px, tbf, cp);
	ps += W;
/*
		{//no error
...
*/
	for(int hh = 1; hh < h; hh++){
		detectRuns512(lk1, ct, ps, px, tbf, cp);
		ps += W;
...
__global__ void pass2_512(ushort* ib, uint* glabel, uint h, uint pitch)
{
...
__global__ void pass3_512(ushort* pt, ushort* ps, ushort* b, uint* label, uint h)
{
...
void label_512(CudaBuffer* cbpt, CudaBuffer* cbpt2, CudaBuffer* cbps, CudaBuffer* cbb, CudaBuffer* cbb2, CudaBuffer* cbglabel, uint h, uint bn, CudaBuffer* cbeb)
{
	...
	pass1_512<<<grid, threads>>>(pt2, ps, b, b2, (h-2)/bn, eb);
	pass2_512<<<grid2, threads2>>>(b, glabel, bn, 1);
	pass3_512<<<grid, threads>>>(pt, pt2, b, glabel, (h-2)/(bn*2));
}
