...
__device__ __inline__ void detectRuns2048(ushort* pt, uchar* ct, uchar* ps, uchar* px, ushort* tbf, uint cp2)
{
	uint* ps4 = (uint*)ps;
	uint* pt2 = (uint*)pt;
...
__global__ void pass1_2048(ushort* pt, uchar* ps, ushort* b,  ushort* b2, uint h, ushort* eb)
{
...
//		return;
	...
	px[cp>>1] = 0;
//	((ushort*)ct)[cp>>1] = 0;
	detectRuns2048(lk0, ct, ps, px, tbf, cp);
	ps += W;
/*
		{//no error
...
*/
	for(int hh = 1; hh < h; hh++){
		detectRuns2048(lk1, ct, ps, px, tbf, cp);
		ps += W;
...
__global__ void pass2_2048(ushort* ib, uint* glabel, uint h, uint pitch)
{
...
__global__ void pass3_2048(ushort* pt, ushort* ps, ushort* b, uint* label, uint h)
{
...
void label_2048(CudaBuffer* cbpt, CudaBuffer* cbpt2, CudaBuffer* cbps, CudaBuffer* cbb, CudaBuffer* cbb2, CudaBuffer* cbglabel, uint h, uint bn, CudaBuffer* cbeb)
{
	ushort* pt = (ushort*)cbpt->GetData();
	ushort* pt2 = (ushort*)cbpt2->GetData();
	uchar* ps = (uchar*)cbps->GetData();
	ushort* b = (ushort*)cbb->GetData();
	ushort* b2 = (ushort*)cbb2->GetData();
	uint* glabel = (uint*)cbglabel->GetData();
	ushort* eb = (ushort*)cbeb->GetData();
	...
	pass1_2048<<<grid, threads>>>(pt2, ps, b, b2, (h-2)/bn, eb);
	pass2_2048<<<grid2, threads2>>>(b, glabel, bn, 1);
	pass3_2048<<<grid, threads>>>(pt, pt2, b, glabel, (h-2)/(bn*2));
}
