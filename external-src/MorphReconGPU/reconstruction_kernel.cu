...
__global__ void
Rec1DForward_X_dilation ( float* g_marker, const float* g_mask, int sx, int sy, int sz, bool* change )
{
...
__global__ void
Rec1DForward_Y_dilation ( float* g_marker, const float* g_mask, int sx, int sy, int sz, bool* change )
{
...
__global__ void
Rec1DForward_Z_dilation ( float* g_marker, const float* g_mask, int sx, int sy, int sz, bool* change )
{
	...
	if ( ((bx + tx) < sx) && ((by + ty) < sy) ) {
		...
		for (int tz = 1; tz < sz; tz++) {
			...
			// perform iteration
			float s_old = s_marker_B[tx][ty];
			s_marker_B[tx][ty] = fmaxf( s_marker_A[tx][ty], s_marker_B[tx][ty] );
			s_marker_B[tx][ty] = fminf( s_marker_B[tx][ty], s_mask    [tx][ty] );
...
__global__ void
Rec1DBackward_X_dilation ( float* g_marker, const float* g_mask, int sx, int sy, int sz, bool* change )
{
...
__global__ void
Rec1DBackward_Y_dilation ( float* g_marker, const float* g_mask, int sx, int sy, int sz, bool* change )
{
...
__global__ void
Rec1DBackward_Z_dilation ( float* g_marker, const float* g_mask, int sx, int sy, int sz, bool* change )
{
...
void
reconstruction_by_dilation_kernel( float* g_marker, const float* g_mask, const int sx, const int sy, const int sz ) 
{
  ...
  bool *h_change, *d_change;
	...
	cutilSafeCall( cudaMalloc( (void**) &d_change, sizeof(bool) ) );
	...
	*h_change = true;
	...
	while ( (*h_change) && (iter < 100000) )  // repeat until stability
	{
		...
		*h_change = false;
		...
		cutilSafeCall( cudaMemcpy( h_change, d_change, sizeof(bool), cudaMemcpyDeviceToHost ) );

	}
	
	cutilSafeCall( cudaFree(d_change) );
	free(h_change);
...
__global__ void
Rec1DForward_X_erosion ( float* g_marker, const float* g_mask, int sx, int sy, int sz, bool* change )
{
	...
	const int bz = blockIdx.y;
		...
		if (ty + by < sy) {
			...
			for (startx = 0; startx < sx - BLOCK_SIZE; startx += BLOCK_SIZE - 1) {
				...
				for (int ix = 1; ix < BLOCK_SIZE; ix++) {
					float s_old = s_marker[ix][ty];
					s_marker[ix][ty] = fminf( s_marker[ix][ty], s_marker[ix-1][ty] );
					s_marker[ix][ty] = fmaxf( s_marker[ix][ty], s_mask  [ix]  [ty] );
					s_change[ix][ty] |= NEQ( s_old, s_marker[ix][ty] );
					__syncthreads();
				}
...
__global__ void
Rec1DForward_Y_erosion ( float* g_marker, const float* g_mask, int sx, int sy, int sz, bool* change )
{
	...
	if ( ((bx + tx) < sx) && ((bz + tz) < sz) ) {
		...
		for (int ty = 1; ty < sy; ty++) {
			...
			// perform iteration
			float s_old = s_marker_B[tx][tz];
			s_marker_B[tx][tz] = fminf( s_marker_A[tx][tz], s_marker_B[tx][tz] );
			s_marker_B[tx][tz] = fmaxf( s_marker_B[tx][tz], s_mask    [tx][tz] );
			s_change[tx][tz] |= NEQ( s_old, s_marker_B[tx][tz] );
			__syncthreads();
...
__global__ void
Rec1DForward_Z_erosion ( float* g_marker, const float* g_mask, int sx, int sy, int sz, bool* change )
{
	...
	if ( ((bx + tx) < sx) && ((by + ty) < sy) ) {
		...
		for (int tz = 1; tz < sz; tz++) {
			...
			// perform iteration
			float s_old = s_marker_B[tx][ty];
			s_marker_B[tx][ty] = fminf( s_marker_A[tx][ty], s_marker_B[tx][ty] );
			s_marker_B[tx][ty] = fmaxf( s_marker_B[tx][ty], s_mask    [tx][ty] );
			s_change[tx][ty] |= NEQ( s_old, s_marker_B[tx][ty] );
			__syncthreads();
...
__global__ void
Rec1DBackward_X_erosion ( float* g_marker, const float* g_mask, int sx, int sy, int sz, bool* change )
{
...
__global__ void
Rec1DBackward_Y_erosion ( float* g_marker, const float* g_mask, int sx, int sy, int sz, bool* change )
{
...
__global__ void
Rec1DBackward_Z_erosion ( float* g_marker, const float* g_mask, int sx, int sy, int sz, bool* change )
{
...
void
reconstruction_by_erosion_kernel( float* g_marker, const float* g_mask, const int sx, const int sy, const int sz ) 
{
	...
	*h_change = true;
	...
	while ( (*h_change) && (iter < 100000) )  // repeat until stability
	{
		...
		*h_change = false;
		...
		// dopredny pruchod pres osu X
		Rec1DForward_X_erosion <<< blocksx, threads >>> ( g_marker, g_mask, sx, sy, sz, d_change );

		// dopredny pruchod pres osu Y
		Rec1DForward_Y_erosion <<< blocksy, threads >>> ( g_marker, g_mask, sx, sy, sz, d_change );

		// dopredny pruchod pres osu Z
		Rec1DForward_Z_erosion <<< blocksz, threads >>> ( g_marker, g_mask, sx, sy, sz, d_change );
		
		// zpetny pruchod pres osu X
		Rec1DBackward_X_erosion<<< blocksx, threads >>> ( g_marker, g_mask, sx, sy, sz, d_change );

		// zpetny pruchod pres osu Y
		Rec1DBackward_Y_erosion<<< blocksy, threads >>> ( g_marker, g_mask, sx, sy, sz, d_change );

		// zpetny pruchod pres osu Z
		Rec1DBackward_Z_erosion<<< blocksz, threads >>> ( g_marker, g_mask, sx, sy, sz, d_change );
